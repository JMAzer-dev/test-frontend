  ...
  return (
    <section className="flex flex-col w-[300px] sm:w-[320px] shadow-none rounded-none">
      <div className=" flex flex-col justify-center text-center items-center text-white">
        <Link to={channelDetail ? `/channel/${channelDetail?.id?.channelId}` : `/demo-video/`}>
          <img src={channelDetail?.snippet?.thumbnails?.medium?.url} alt={channelDetail?.snippet?.title} className='rounded-full w-[180px]  '/>
        </Link>
